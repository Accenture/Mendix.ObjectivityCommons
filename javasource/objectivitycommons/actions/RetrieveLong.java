...
import objectivitycommons.proxies.NumericValue;
 ...
 */
public class RetrieveLong extends CustomJavaAction<java.util.List<IMendixObject>>
{
	...
	{
		// BEGIN USER CODE
		if(AttributeName == null || AttributeName.isEmpty()) throw new MendixRuntimeException("AttributeName cannot be empty");
		...
		xp = String.format("//%s %s /%s", Entity, xp, AttributeName);
        ...
        logger.debug(String.format("Calling retrieveXPathQueryRaw: amount=%d; offset=%d; xpath:%s; sort: %s",
			amount, offset, xp, sortMap.toString()));

		IDataTable resultTable = Core.retrieveXPathQueryRaw(
			ctx,
			xp,
			amount,
			offset,
		...
		resultTable.forEach(row -> {
			NumericValue dto = new NumericValue(ctx);
			dto.setValue(row.getValue(ctx, AttributeName));
			resultList.add(dto.getMendixObject());
		});
		return resultList;
		// END USER CODE
	}
